...
import buildsrc.utils.titlecaseFirstChar
...
plugins.withType<MavenPublishPlugin>().all {
  extensions
    ...
    .withType<MavenPublication>().all publication@{
      ...
      val installTaskName =
        "publish${publicationName.titlecaseFirstChar()}PublicationToTestMavenRepo"

      // Register a publication task for each publication.
      // Use PublishToMavenLocal, because the PublishToMavenRepository task will *always*
      // append a timestamp to the JAR for SNAPSHOT versions, even if no code has changed.
